...
class TinyGsmSim7670 : public TinyGsmModem<TinyGsmSim7670>,
 ...
 protected:
   ...
   */
 public:
  SIM7670RegStatus getRegistrationStatus() {
    return (SIM7670RegStatus)getRegistrationStatusXREG("CGREG");
  }

 protected:
  bool isNetworkConnectedImpl() {
    SIM7670RegStatus s = getRegistrationStatus();
 ...
 protected:
  // enable GPS
  bool enableGPSImpl() {
    uint32_t timeout_ms = 30 * 1000;
    sendAT(GF("+CGNSSPWR=0"));
    waitResponse() ;
    delay(5000);
    sendAT(GF("+CGNSSPWR=1"));
    if (waitResponse(timeout_ms) != 1) { return false; }
    waitResponse(timeout_ms, GF(AT_NL "+CGNSSPWR: READY!"));
    delay(5000);
    sendAT(GF("+CGNSSMODE=1"));
    if (waitResponse(timeout_ms) != 1) { return false; }
